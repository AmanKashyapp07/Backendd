...
title: "Caching"
navTitle: "Caching"
summary: "Understanding the mechanism that makes high-performance backend systems fast, from Google Search to Netflix to Redis."
...
Below are practical, production-ready implementations of caching patterns using modern JavaScript (Node.js & Express) with Redis.

### 10.1: Lazy (Cache-Aside) Caching in Node.js

```js title="cache-aside.js / Cache-Aside pattern with Node.js and ioredis"
...
async function fetchProductFromDatabase(productId) {
  // e.g. await db.query('SELECT * FROM products WHERE id = $1', [productId]);
  return { id: productId, name: 'Mechanical Keyboard', price: 99.99 };
}

// 1. CACHE-ASIDE (Lazy Caching):
// Step 1: Check Redis first (Cache Hit path)
// Step 2: On miss, query database, store result in Redis with TTL, return
export async function getProduct(productId) {
  ...
  if (cached) {
    ...
    return JSON.parse(cached); // fast in-memory deserialization
  }

  // Step 2: Cache Miss -> query primary source of truth (DB)
  console.log('[CACHE MISS] Fetching from database...', productId);
  const product = await fetchProductFromDatabase(productId);
  if (!product) return null;

  // Step 3: Populate cache with 1-hour TTL to prevent stale accumulation
  await redis.setex(cacheKey, ONE_HOUR, JSON.stringify(product));

  return product;
}

// 2. WRITE-THROUGH / CACHE INVALIDATION:
// When state changes, update DB and invalidate or update cache immediately
export async function updateProduct(product) {
  // Step 1: Update database
  // await db.query('UPDATE products SET name = $1, price = $2 WHERE id = $3', [product.name, product.price, product.id]);

  // Step 2: Invalidate cache (or set new value with TTL)
...
### 10.2: Rate Limiting Middleware in Express

```js title="rate-limiter.js / Atomic rate limiter middleware in Express with Redis"
import { Redis } from 'ioredis';
...
export function rateLimitMiddleware(req, res, next) {
  // Extract client IP (respecting proxy headers if behind Nginx/Cloudflare)
  const clientIp = req.headers['x-forwarded-for']?.split(',')[0].trim() || req.socket.remoteAddress;

  // Window timestamp key (bucketed per minute)
  const currentMinute = Math.floor(Date.now() / 1000 / WINDOW_SECONDS);
  const key = `rate_limit:${clientIp}:${currentMinute}`;

  // Atomic pipeline: increment counter and set expiry if newly created
  redis.multi()
    .incr(key)
    .expire(key, WINDOW_SECONDS)
    .exec(async (err, results) => {
      if (err) {
        return next(); // Fail-open on Redis errors so users aren't blocked
      }
...
### 10.3: Route Caching Middleware in Express

```js title="route-cache.js / Reusable response caching middleware in Express"
import { Redis } from 'ioredis';

const redis = new Redis(process.env.REDIS_URL || 'redis://localhost:6379');

// Higher-order middleware factory for route-level caching
export function cacheRoute(ttlSeconds = 3600) {
  return async (req, res, next) => {
    ...
    try {
      ...
      // Step 2: Cache Miss -> Intercept res.json() to capture output
      res.setHeader('X-Cache', 'MISS');
      const originalJson = res.json.bind(res);

      res.json = (body) => {
        // Asynchronously save to Redis with TTL
      ...
      next(); // fallback to normal execution if cache read fails
    }
  };
}

// Example usage in Express application:
// app.get('/api/products/:id', cacheRoute(3600), async (req, res) => {
//   const product = await fetchProductFromDB(req.params.id);
//   res.json(product);
// });
```

### 10.4: Session Management with Redis in Express

```js title="session-store.js / Redis session store with sliding window TTL in Express"
...
export async function createSession(userId, metadata = {}) {
  ...
  // setex guarantees automatic expiration; no manual cleanup cron jobs needed
  await redis.setex(sessionKey, SESSION_TTL_SECONDS, JSON.stringify(payload));
  return sessionId; // returned to client in HttpOnly cookie
}

// Retrieve and validate session on each authenticated request
export async function getSession(sessionId) {
  const sessionKey = `session:${sessionId}`;
  const data = await redis.get(sessionKey);

  if (!data) {
    return null; // session expired or nonexistent
  }

  // Sliding window expiration: refresh TTL on active user requests
...
<Callout type="info">
[ioredis GitHub & Documentation](https://github.com/redis/ioredis) [node-redis Official NPM Package](https://www.npmjs.com/package/redis) [Redis Official Node.js Quickstart](https://redis.io/docs/latest/develop/connect/clients/nodejs/)
</Callout>
...
Notes compiled from lecture transcript / Modern JavaScript (Node.js & Express) examples / MDN & Redis references inline

Backend from First Principles / Chapter 09 / Caching. Code targets Node.js 20+ and Express 5.

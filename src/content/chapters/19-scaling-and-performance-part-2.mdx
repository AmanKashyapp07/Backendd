...
| **Background jobs** | In-process timer / worker thread | BullMQ / Redis queue, RabbitMQ, Kafka              |

#### Stateless Session Management in Express with Redis

```js title="session.js / Stateless Redis session store in Express"
...
export async function createSession(userId, res) {
  ...
  // Store session in Redis with 24-hour expiration (stateless application tier)
  await redis.setex(`session:${sessionId}`, 86400, JSON.stringify({ userId }));

  // Return HttpOnly cookie to client
...
```js title="health.js / Health check endpoint for load balancers"
import express from 'express';

const app = express();

// Load balancers poll this to route traffic only to healthy instances
...
#### Background Jobs with BullMQ & Redis in Node.js

```js title="queue.js / Background task queue with BullMQ"
import { Queue, Worker } from 'bullmq';

const connection = { host: 'localhost', port: 6379 };

// 1. Producer: Enqueue task without blocking HTTP response
...
export async function enqueueEmail(email, inviteUrl) {
  // Returns instantly; does not block the caller
  return emailQueue.add('sendInvite', { email, inviteUrl });
}

// 2. Worker: Process jobs in the background
export const emailWorker = new Worker(
  'emails',
  async (job) => {
    const { email, inviteUrl } = job.data;
    console.log(`Sending invite email to ${email}...`);
    // Simulated third-party email provider call (300ms)
...
<Callout type="info">
**Reference:** [BullMQ (Node.js)](https://docs.bullmq.io/), production-grade distributed task queue for JavaScript and TypeScript.
</Callout>
...
| AWS Lambda (Node.js) | Firecracker + V8 Engine  | \~100-250ms   |
...
```js title="observability.js / Prometheus metrics instrumentation in Node.js"
import client from 'prom-client';
import express from 'express';

const app = express();

// Track HTTP request duration across methods, routes, and status codes
...
Backend from First Principles / Chapter 19 / Scaling II. Code targets Node.js 20+ and Express 5.
